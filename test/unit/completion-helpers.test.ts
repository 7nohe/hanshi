...
import {
	...
	sanitizeCompletion,
} from "../../src/ai/completion-helpers";
...
const context = {
	...
	relatedFiles: [
		{ path: "appendix.md", excerpt: "# Appendix\n\nMore details." },
	],
};

describe("sanitizeCompletion", () => {
	it("drops repeated current-line prefixes", () => {
		expect(sanitizeCompletion("console.log(value)", context)).toBe(
			"log(value)",
		);
	});
	...
	it("stops before a new markdown block starts", () => {
		expect(
			sanitizeCompletion("This continues the paragraph.\n## Appendix", context),
		).toBe("This continues the paragraph.");
	});

	it("keeps inline completions on a single line in the middle of text", () => {
		expect(sanitizeCompletion("log(value)\nreturn value", context)).toBe(
			"log(value)",
		);
	});

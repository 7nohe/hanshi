...
suite("Hanshi extension integration", () => {
				...
				return vscode.window.tabGroups.all.some((group) =>
					group.tabs.some((tab) => {
						...
						return (
							...
							input?.viewType?.includes(VIEW_TYPE)
						);
					}),
				);
			});
	...
	test("aiCompletions.enabled configuration default is true", () => {
		const value = vscode.workspace
			.getConfiguration("hanshi")
			.get("aiCompletions.enabled");
	...
	test("registers the hanshi_getSelection language model tool", function () {
		...
		assert.ok(
			names.includes("hanshi_getSelection"),
			`expected tool; got ${names.join(", ")}`,
		);
	});
});

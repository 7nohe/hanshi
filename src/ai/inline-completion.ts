...
import { buildCompletionPrompt, sanitizeCompletion } from './completion-helpers';
import type {
...
export class InlineCompletionService {
  ...
  public async handleRequest(message: RequestCompletionMessage): Promise<void> {
    ...
    try {
      ...
      const response = await model.sendRequest(
        [vscode.LanguageModelChatMessage.User(buildCompletionPrompt(message.markdown, message.context))],
        {
      ...
      if (!this.isLatest(message.requestId, message.version) || tokenSource.token.isCancellationRequested) {
        return;
      }
      ...
      await this.options.postMessage(createCompletionResult(message, insertText));
    } catch (error) {
  ...
  public async cancel(requestId: string | undefined, version: number): Promise<void> {
    if (requestId && this.latestRequestId && requestId !== this.latestRequestId) {
      return;
    }
  ...
  private async resolveModel(): Promise<vscode.LanguageModelChat | undefined> {
    const selectChatModels = this.options.selectChatModels ?? vscode.lm.selectChatModels;
    ...
    const canSend = this.options.languageModelAccessInformation.canSendRequest(model);
  ...
  private async postCleared(requestId: string | undefined, version: number): Promise<void> {

...
export function renderMermaidPreview(
  ...
  void loadMermaid()
    .then((mermaid) => mermaid.default.render(`hanshi-mermaid-${nextRenderId++}`, source))
    .then(({ svg }) => {
    ...
    .catch((error: unknown) => {
      ...
      applyPreview(`<div class="hanshi-mermaid-preview is-error">${escapeHtml(message)}</div>`);
    });
}
...
function installDelegation(): void {
  ...
  document.addEventListener('click', (e) => {
    const btn = (e.target as HTMLElement).closest<HTMLElement>('.hanshi-mermaid-btn[data-zoom]');
    if (!btn) return;

    const viewport = btn.closest('.hanshi-mermaid-preview')?.querySelector<HTMLElement>('.hanshi-mermaid-viewport');
    if (!viewport) return;
    ...
    if (direction === 0) {
      ...
      setViewportTransform(viewport, clampScale(current + direction * ZOOM_STEP));
    }
  });

  document.addEventListener('wheel', (e) => {
    const viewport = (e.target as HTMLElement).closest<HTMLElement>('.hanshi-mermaid-viewport');
    if (!viewport) return;
  ...
  document.addEventListener('pointerup', () => { dragViewport = null; });
  document.addEventListener('pointercancel', () => { dragViewport = null; });
}
...
function setViewportTransform(
  ...
  const label = viewport.closest('.hanshi-mermaid-preview')?.querySelector<HTMLElement>('.hanshi-mermaid-zoom-label');
  if (label) {

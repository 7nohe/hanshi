...
interface CompletionControllerOptions {
	...
	postMessage: (
		message:
			| RequestCompletionMessage
			| { type: "cancelCompletion"; requestId?: string; version: number },
	) => void;
...
export class CompletionController {
	...
	public applyCompletionResult(message: CompletionResultMessage): void {
		if (
			message.requestId !== this.pendingRequestId ||
			message.version !== this.options.getVersion()
		) {
			return;
		}
	...
	private renderActiveCompletion(): void {
		...
		this.overlay.style.maxHeight = availableHeight
			? `${availableHeight}px`
			: "";
	...
	private buildContext(): CompletionContext | undefined {
		...
		const before = blockText.slice(
			Math.max(0, blockPrefix.length - 240),
			blockPrefix.length,
		);
...
function findLineElement(node: Node, root: HTMLElement): HTMLElement | null {
	...
	return (
		element?.closest(
			".cm-line, p, li, h1, h2, h3, h4, h5, h6, blockquote, pre",
...
function findBlockElement(node: Node, root: HTMLElement): HTMLElement | null {
	...
	return (
		element?.closest(
			".cm-line, p, li, h1, h2, h3, h4, h5, h6, blockquote, pre, td, th",
		) ?? root.querySelector(".ProseMirror")
	);
}
...
function computeAvailableHeight(
	...
	const blockParent = lineElement.closest(

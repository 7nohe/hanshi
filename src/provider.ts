...
export class HanshiEditorProvider
	implements vscode.CustomEditorProvider<HanshiDocument>
{
	public static readonly viewType = "hanshi.markdownEditor";
	private readonly relatedFilesCache = new Map<
		string,
		Array<{ path: string; excerpt: string }>
	>();
	private readonly onDidChangeCustomDocumentEmitter = new vscode.EventEmitter<
		vscode.CustomDocumentEditEvent<HanshiDocument>
	>();
	...
	private static selectionCallbacks = new Map<
		string,
		(response: SelectionResponseMessage) => void
	>();
	...
	public static async getSelection(): Promise<SelectionResult | undefined> {
		if (
			!HanshiEditorProvider.activeWebview ||
			!HanshiEditorProvider.activeDocument
		) {
			return undefined;
		}

		const requestId = `sel-${++HanshiEditorProvider.requestSequence}`;
		const webview = HanshiEditorProvider.activeWebview;
		const document = HanshiEditorProvider.activeDocument;

		const response = await new Promise<SelectionResponseMessage | undefined>(
			(resolve) => {
				const timer = setTimeout(() => {
					HanshiEditorProvider.selectionCallbacks.delete(requestId);
					resolve(undefined);
				}, 3000);
				HanshiEditorProvider.selectionCallbacks.set(requestId, (msg) => {
					clearTimeout(timer);
					resolve(msg);
				});
	...
	private getCompletionsEnabled(document: TextBackedDocument): boolean {
		return vscode.workspace
			.getConfiguration("hanshi", document.uri)
				...
				try {
					switch (message.type) {
						...
						case "requestCompletion":
							await completions.handleRequest(
								await this.enrichCompletionRequest(document, message),
							);
							return;
						...
						case "requestHistory":
							await vscode.commands.executeCommand(
								message.direction === "undo" ? "undo" : "redo",
							);
							return;
						case "selectionResponse": {
							const callback = HanshiEditorProvider.selectionCallbacks.get(
								message.requestId,
							);
							if (callback) {
								HanshiEditorProvider.selectionCallbacks.delete(
									message.requestId,
								);
								callback(message);
							}
	...
	private async showNoticeToDocument(
		document: HanshiDocument,
		message: string,
	): Promise<void> {
		await Promise.allSettled(
	...
	private async handleSaveImage(
		...
		try {
			const { alt, path: relativePath } = await this.persistImage(
				document,
				message.name,
				message.dataUrl,
			);
	...
	private async persistImage(
		...
		const relativePath = path
			.relative(documentDirectory, imageUri.fsPath)
			.split(path.sep)
			.join("/");
		return {
	...
	private resolveImageSrc(
		...
		if (
			/^(?:[a-z]+:)?\/\//i.test(src) ||
			/^(?:data|blob|vscode-webview):/i.test(src)
		) {
			return src;
		}
		...
		if (
			!resolvedPath.startsWith(documentDirectory + path.sep) &&
			resolvedPath !== documentDirectory
		) {
	...
	private async enrichCompletionRequest(
		...
		const relatedFiles = await this.collectRelatedMarkdownFiles(
			document,
			message.markdown,
		);
	...
	private async collectRelatedMarkdownFiles(
		...
		if (collected.size < 3) {
			...
			const siblingUris = siblingFiles
				.filter((s) => s.fsPath !== document.uri.fsPath)
				.slice(0, 3 - collected.size);

			const siblingResults = await Promise.all(
				siblingUris.map((sibling) =>
					this.readRelatedMarkdownFile(document, sibling),
				),
			);
	...
	private async readRelatedMarkdownFile(
		...
		try {
			...
			return {
				path: path
					.relative(path.dirname(document.uri.fsPath), uri.fsPath)
					.split(path.sep)
					.join("/"),
...
function mimeSubtypeToExtension(subtype: string): string {
	return (
		...
		(subtype.replace(/[^a-zA-Z0-9]/g, "") || "png")
	);
}

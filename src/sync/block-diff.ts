...
export function computeBlockReplaceRange(
	current: string,
	next: string,
): ReplaceSpan | undefined {
	...
	while (prefixCount < minLen) {
		...
		if (!cb || !nb || cb.type !== nb.type || cb.segmentText !== nb.segmentText)
			break;
		prefixCount++;
	}
	...
	if (
		prefixCount + suffixCount >= currentBlocks.length &&
		prefixCount + suffixCount >= nextBlocks.length
	) {
	...
	const currentMiddle = currentBlocks.slice(
		prefixCount,
		currentBlocks.length - suffixCount,
	);
	const nextMiddle = nextBlocks.slice(
		prefixCount,
		nextBlocks.length - suffixCount,
	);
	...
	const currentStart =
		prefixCount < currentBlocks.length
			? currentBlocks[prefixCount]?.start ?? current.length
			: current.length;
	const currentEnd =
		suffixCount > 0
			? currentBlocks[currentBlocks.length - suffixCount]?.start ?? current.length
			: current.length;

	const nextFirst = prefixCount;
	const nextLast = nextBlocks.length - suffixCount;
	const nextText = nextBlocks
		.slice(nextFirst, nextLast)
		.map((b) => b.segmentText)
		.join("");
